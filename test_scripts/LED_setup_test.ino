...
int leftLEDs[12][4] = {{0,0,0,1}, {0,0,0,2}, {0,0,0,4}, {0,0,0,8},
                    {0,0,0,16}, {0,0,0,32}, {0,0,0,64}, {0,0,0,128},
                    {0,0,1,0}, {0,0,2,0}, {0,0,4,0}, {0,0,8,0}};

int rightLEDs[12][4] = {{0,0,16,0}, {0,0,32,0}, {0,0,64,0}, {0,0,128,0},
                    {0,1,0,0}, {0,2,0,32}, {0,4,0,0}, {0,8,0,0},
                    {0,16,0,0}, {0,32,0,0}, {0,64,0,0}, {0,128,0,0}};

int midLEDs[6][4] = {{1,0,0,0}, {2,0,0,0}, {4,0,0,0}, {8,0,0,0},
                     {16,0,0,0}, {32,0,0,0}};
...
void loop() {
  ...
  if (currState == "left") {
    ...
    if (prevState == "left") {
      shiftOut(dataPin, clockPin, MSBFIRST, midLEDs[i][0] + leftLEDs[prevPos][0]);
      shiftOut(dataPin, clockPin, MSBFIRST, midLEDs[i][1] + leftLEDs[prevPos][1]);
      shiftOut(dataPin, clockPin, MSBFIRST, midLEDs[i][2] + leftLEDs[prevPos][2]);
      shiftOut(dataPin, clockPin, MSBFIRST, midLEDs[i][3] + leftLEDs[prevPos][3]);
    } else if (prevState == "right") {
      j = 5-i;
      shiftOut(dataPin, clockPin, MSBFIRST, midLEDs[j][0] + rightLEDs[prevPos][0]);
      shiftOut(dataPin, clockPin, MSBFIRST, midLEDs[j][1] + rightLEDs[prevPos][1]);
      shiftOut(dataPin, clockPin, MSBFIRST, midLEDs[j][2] + rightLEDs[prevPos][2]);
      shiftOut(dataPin, clockPin, MSBFIRST, midLEDs[j][3] + rightLEDs[prevPos][3]);
    }
